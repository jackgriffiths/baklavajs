...
export const ZOOM_TO_FIT_NODE_COMMAND = "ZOOM_TO_FIT_NODE";
export const ZOOM_TO_FIT_GRAPH_COMMAND = "ZOOM_TO_FIT_GRAPH";

export type ZoomToFitRectCommand = ICommand<void, [IRect]>;
export type ZoomToFitNodeCommand = ICommand<void, [AbstractNode]>;
...
export function registerZoomToFitCommands(displayedGraph: Ref<Graph>, handler: ICommandHandler, settings: IViewSettings) {
    ...
    handler.registerCommand(ZOOM_TO_FIT_NODE_COMMAND, {
        canExecute: () => true,
        execute: (node: AbstractNode) => zoomToFitNode(displayedGraph.value, settings, node),
    });
...
function zoomToFitNode(graph: Graph, settings: IViewSettings, node: AbstractNode) {
    if (node.graph !== graph) {
        return;
    }

    const nodeRect = getNodeRect(node);
    zoomToFitRect(graph, settings, nodeRect);
}
...
function zoomToFitGraph(graph: Graph, settings: IViewSettings) {
    ...
    const nodeRects = graph.nodes.map(getNodeRect);
